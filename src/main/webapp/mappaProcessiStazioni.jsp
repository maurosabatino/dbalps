    ...
    <body>
        ...
        <script>
            $(document).ready(function () {
                ...
                var marker;
                var marker_list = [];
                var stazioni = (function () {
                    var stazioni = null;
                    $.ajax({
                        async: false,
                        global: false,
                        url: 'servletJson?op=stazioni',
                        dataType: 'json',
                ...
                $.each(processi, function (index, value) {
                 ...
                 $.each(stazioni, function (index, value) {
                    if (value.ubicazione.coordinate.x != 0 || value.ubicazione.coordinate.y != 0) {
                        pos = new google.maps.LatLng(value.ubicazione.coordinate.x, value.ubicazione.coordinate.y);
                        marker = new google.maps.Marker({
                            position: pos,
                        ...
                        google.maps.event.addListener(marker, 'click', (function (marker, index) {
                            return function () {
                                infowindow.setContent("nome: " + value.nome + " <br> comune: " + value.ubicazione.quota+"<br> quota "+value.data+"<br> \n\
